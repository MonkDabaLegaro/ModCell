import type { ApplicationDetails, ApplicationInventory, DeviceDirectoryListing, DeviceSnapshot } from "@modcell/contracts";
...
export const modcellApi = {
  ...
  installApplication: async (serial: string, file: File) => {
    const body = new FormData(); body.append("file", file);
    const response = await fetch(`/api/devices/${encodeURIComponent(serial)}/applications/install`, { method: "POST", body });
    if (!response.ok) { const payload = await response.json().catch(() => null) as { error?: string } | null; throw new Error(payload?.error ?? `Install failed (${response.status})`); }
  },
